...
#' @section Methods:
#' \describe{
#' \strong{ GetAuditLogs } \emph{ Get audit logs }
...
#' \item \emph{ @returnType } \link{AuditLogs} \cr
#'
#' \item On encountering errors, an error of subclass ApiException will be thrown.
#'
#' \item status code : 200 | The audit logs results and paging information
#'
#' \item return type : AuditLogs
...
AuditLogsApi <- R6::R6Class(
  ...
  public = list(
    ...
    #' Get audit logs
    #'
    #' @param limit (optional) Number of logs to return. The minimum supported value is 1 and maximum supported value is 500. Note that `limit` is a \"best effort\" request since there may not be enough logs to satisfy the limit. (default value: 20)
    #' @param previous (optional) Gets the previous page of audit logs relative to the given id.
    #' @param var_next (optional) Gets the next page of audit logs relative to the given id.
    #' @param asc_order (optional) Whether the audit logs should be listed in ascending order (default value: TRUE)
    #' @param data_file (optional) name of the data file to save the result
    #' @param ... Other optional arguments
    #' @return AuditLogs
    #' @export
    GetAuditLogs = function(limit = 20, previous = NULL, var_next = NULL, asc_order = TRUE, data_file = NULL, ...) {
      local_var_response <- self$GetAuditLogsWithHttpInfo(limit, previous, var_next, asc_order, data_file = data_file, ...)
    ...
    #' @param data_file (optional) name of the data file to save the result
    #' @param ... Other optional arguments
    #' @return API response (AuditLogs) with additional information such as HTTP status code, headers
    #' @export
    GetAuditLogsWithHttpInfo = function(limit = 20, previous = NULL, var_next = NULL, asc_order = TRUE, data_file = NULL, ...) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (`limit` > 500) {
        rlang::abort(message = "Invalid value for `limit` when calling AuditLogsApi$GetAuditLogs, must be smaller than or equal to 500.",
                     .subclass = "ApiException",
                     ApiException = ApiException$new(status = 0,
                                                     reason = "Invalid value for `limit` when calling AuditLogsApi$GetAuditLogs, must be smaller than or equal to 500."))
      }
      if (`limit` < 1) {
        rlang::abort(message = "Invalid value for `limit` when calling AuditLogsApi$GetAuditLogs, must be bigger than or equal to 1.",
                     .subclass = "ApiException",
                     ApiException = ApiException$new(status = 0,
                                                     reason = "Invalid value for `limit` when calling AuditLogsApi$GetAuditLogs, must be bigger than or equal to 1."))
      }
      ...
      query_params[["ascOrder"]] <- `asc_order`

      local_var_url_path <- "/v1/audit_logs"
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$deserialize(local_var_resp$response, "AuditLogs", loadNamespace("connectapi2")),
          error = function(e) {
